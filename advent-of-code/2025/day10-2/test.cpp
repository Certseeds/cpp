...
std::string getFilePath() noexcept { return "./../../../../advent-of-code/2025/day10-2/resource/"; }
...
TEST_CASE("test case 1", "[2025 day10-2]") {
    SECTION("do") {
...
TEST_CASE("test case 2", "[2025 day10-2][.]") {
    SECTION("do") {
        const CS203_redirect cr{"02.data.in", "02.test.out"};
        const auto input_data = read();
        const auto output_data = cal(input_data);
        output(output_data);
    }
    SECTION("compare files") {
        CHECK(compareFiles("02.test.out", "02.data.out"));
    }
}
TEST_CASE("test case large", "[2025 day10-2][.]") {
    SECTION("do") {
