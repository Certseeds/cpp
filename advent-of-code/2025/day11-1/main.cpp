...
inline input_type read() {
    ...
    const auto head_index = map["you"];
    return {graph, in_num, head_index};
}

output_type cal(const input_type &datas) {
    const auto &[graph, _in_num, head_index] = datas;
    ...
    std::vector<size_t> times(graph.head.size(), 0);
    times[out_index] = 1;
