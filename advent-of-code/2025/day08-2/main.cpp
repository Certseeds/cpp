...
vector<distance> N2(const input_type &points) {
...
int64_t build_graph_and_find_maximum(size_t sizes, size_t links_num, const vector<distance> &dists) {
    tree::Graph graph(sizes, 2 * links_num + 3);
    for (size_t i = 0; i < links_num; ++i) {
        const auto &dist = dists[i];
        graph.add_undirected_edge(dist.p1, dist.p2);
    }
...
// 对输入二分, 当且仅当正好连通时, 返回最后一次操作的两个
output_type cal(const input_type &datas) {
    const auto dists = N2(datas);
    size_t left = datas.size() - 1; // 至少999次操作才能连接上

    // 标准二分操作, 目的是寻找到最小的 满足 >= 操作的位置
    for (size_t right = dists.size() // 全联通了才连上
         ; left < right
         ;) {
        const auto mid = left + (right - left) / 2;
        const auto mid_value = build_graph_and_find_maximum(datas.size(), mid, dists);
        if (mid_value < datas.size()) {
            left = mid + 1;
        } else {
            right = mid;
        }
    }
    const auto node1 = datas[dists[left - 1].p1];
    const auto node2 = datas[dists[left - 1].p2];
    std::cout << std::format("{} {}", node1.x, node2.x) << next_line;
    const auto multi = node1.x * node2.x;
    return multi;
}
